...
export const CalendarView = ({
  ...
  const currentDayDeadlines = selectedDayForDetails ? getDeadlinesForDay(selectedDayForDetails) : [];
  return <Card className='w-full'>
      <CardHeader className='pb-3'>
        <div className='flex flex-col sm:flex-row sm:items-center justify-between gap-3'>
          ...
          <div className='flex items-center justify-center sm:justify-end space-x-2'>
            ...
            {!isCurrentMonthDisplayed && <Button variant='secondary' size='sm' onClick={goToCurrentMonth} className='ml-2 text-xs sm:text-sm px-3 py-1 sm:px-4 sm:py-2'>
                Today
              </Button>}
          </div>
        </div>
      ...
      <CardContent className='p-2 sm:p-6'>
        ...
        <div className='grid grid-cols-7 gap-1 sm:gap-2 mb-2 sm:mb-4'>
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(day => <div key={day} className='p-1 sm:p-2 text-center font-semibold text-muted-foreground text-xs sm:text-sm'>
        ...
        <div className='grid grid-cols-7 gap-1 sm:gap-2'>
          ...
          return <div key={index} className={cn("min-h-[60px] sm:min-h-[100px] p-1 sm:p-2 border rounded-lg transition-colors cursor-pointer", !isCurrentMonth && "opacity-50 text-muted-foreground", isToday(date) && "bg-blue-100 border-blue-300", hasDeadlines && isCurrentMonth && "bg-accent/30 hover:bg-accent/50", "flex flex-col items-center sm:items-start overflow-hidden relative")} onClick={() => openDayDetailModal(date)}>
                <div className={cn("text-xs sm:text-sm font-medium mb-1", isToday(date) && "text-blue-600")}>
                  {format(date, "d")}
                </div>

                {/* Visual dots for deadlines (max 3, then +N) */}
                {hasDeadlines && <div className='flex flex-wrap justify-center sm:justify-start gap-0.5 sm:gap-1 mt-1'>
                    {dayDeadlines.slice(0, 3).map(deadline => <span key={deadline.id} className={cn("w-2 h-2 rounded-full", getDotColor(deadline))} title={deadline.title} />)}
                    {dayDeadlines.length > 3 && <span className='text-[8px] sm:text-xs text-muted-foreground'>
                ...
                <div className='hidden sm:block space-y-0.5 sm:space-y-1 mt-1 w-full overflow-hidden'>
                  {dayDeadlines.slice(0, 2).map(deadline => <div key={deadline.id} className={cn("text-[10px] sm:text-xs p-0.5 sm:p-1 rounded text-white truncate w-full", getDotColor(deadline), deadline.completed && "opacity-60 line-through")} title={`${deadline.title} - ${deadline.subject}`}>
                      {deadline.title}
                    </div>)}
                  {dayDeadlines.length > 2 && <div className='text-[10px] sm:text-xs text-muted-foreground text-center sm:text-left w-full'>
        ...
        <DialogContent className='w-[calc(100vw-2rem)] max-h-[calc(100vh-2rem)] overflow-y-auto 
          ...
          <DialogHeader>
            <DialogTitle className='flex items-center gap-2'>
              ...
              Deadlines for{" "}
              {selectedDayForDetails ? format(selectedDayForDetails, "PPP") : "Selected Day"}
            </DialogTitle>
            <DialogDescription>
          ...
          <div className='py-4 space-y-4'>
            {currentDayDeadlines.length > 0 ? currentDayDeadlines.map(deadline => <div key={deadline.id} className={cn("p-3 border rounded-lg", getDotColor(deadline).replace("bg-", "border-") + " border", deadline.completed ? "bg-muted text-muted-foreground opacity-80" : "bg-card text-card-foreground")}>
                  <h3 className={cn("font-semibold text-lg", deadline.completed && "line-through text-muted-foreground")}>
                    {deadline.title}
                  </h3>
                  <p className='text-sm text-muted-foreground'>
                    {deadline.subject} •{" "}
                    {deadline.type === "quiz" ? "quiz" : deadline.type === "assignment" ? "assignment" : deadline.type === "mid" ? "mid" : deadline.type === "presentation" ? "presentation" : "final"}
                  </p>
                  <p className='text-sm text-muted-foreground'>
                    Priority:{" "}
                  ...
                  {deadline.description && <p className='text-xs mt-2 p-2 bg-background/50 rounded-md'>
